<x-app-layout>
    ...
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        ...
        <div class="card px-4 py-4 sm:px-5">
            <form x-data action="{{route('plans.update', $plan->id)}}" method="post">
                @csrf
                @method('put')
                <x-input type="text" placeholder="Nome" :value="old('name', $plan->name)" name="name" />
                <x-input type="number" placeholder="Quantidade de instâncias" :value="old('quantity_instance', $plan->quantity_instance)" name="quantity_instance" />
                <x-input type="number" placeholder="Quantidade de mensagens" :value="old('quantity_messages', $plan->quantity_messages)" name="quantity_messages" />
                <small>Usando quantidade 0 as mensagens serão ilimitadas</small>
                <x-input type="text" placeholder="Preço" x-mask:dynamic="$money($input, ',','.', 2)" :value="old('price', $plan->price)" name="price" />
                <x-input.checkbox label="Ativo" name="is_active" :checked="$plan->is_active" />    
