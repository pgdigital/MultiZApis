        ...
        <div class="is-scrollbar-hidden flex grow flex-col space-y-4 overflow-y-auto pt-6">
            ...
            @can('viewAny', \App\Models\Plan::class)    
                <a href="{{route('plans.index')}}"
                    ...
                    <div class="h-full w-full flex justify-center items-center mt-3">
                        <i class="fa-solid fa-list-alt text-lg text-center size-7"></i>
                    </div>
                </a>
            @endcan
            {{-- <a href="{{route('campaigns.index')}}"
                class="flex size-11 items-center justify-center rounded-lg outline-none transition-colors duration-200 {{ request()->routeIs('instances.*') ? 'text-primary hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:bg-navy-600 bg-primary/10 dark:text-accent-light dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90' : 'hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25' }}"
                x-tooltip.placement.right="'Campanhas'">
                <div class="h-full w-full flex justify-center items-center mt-3">
                    <i class="fa-solid fa-newspaper text-lg text-center size-7"></i>
                </div>
            </a> --}}
            ...
            @can('viewAny', \App\Models\Module::class)
                <a href="{{route('integrations.index')}}"
                    class="flex size-11 items-center justify-center rounded-lg outline-none transition-colors duration-200 {{ request()->routeIs('integrations.*') ? 'text-primary hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:bg-navy-600 bg-primary/10 dark:text-accent-light dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90' : 'hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25' }}"
                    x-tooltip.placement.right="'Plugins'">
                    <div class="h-full w-full flex justify-center items-center mt-3">
                        <i class="fas fa-puzzle-piece text-lg text-center size-7"></i>
                    </div>
                </a>
            @endcan
        </div>
        ...
        <div class="flex flex-col items-center space-y-3 py-3">
            ...
            <div x-data="usePopper({ placement: 'right-end', offset: 12 })" @click.outside="if(isShowPopper) isShowPopper = false" class="flex">
                <button @click="isShowPopper = !isShowPopper" x-ref="popperRef" class="avatar size-12">
                    <img class="rounded-full" src="{{auth()->user()->avatar_url}}" alt="avatar" />
                    <span
